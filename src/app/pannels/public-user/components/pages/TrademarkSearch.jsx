...
const TrademarkSearch = () => {
  ...
  const debouncedClassDetailSearchTerm = useDebounce(
    classDetailSearchTerm,
    2000
  );
  ...
  const restoreFocus = useCallback(() => {
    setTimeout(() => {
      if (activeInputRef.current === "brand" && brandSearchInputRef.current) {
        ...
        activeInputRef.current === "classDetail" &&
        classDetailSearchInputRef.current
      ) {
      ...
      try {
        ...
        if (results.length > 0) {
          if (hasSearchTerm && !hasClassSelected) {
            ...
            const sortedClasses = Object.values(classesFound).sort(
              (a, b) => parseInt(a.number, 10) - parseInt(b.number, 10)
            );
            setRelevantClasses(sortedClasses);

            if (sortedClasses.length > 0) {
              const classNumbersList = sortedClasses
                .map((c) => c.number)
                .join(", ");
              setMessage(
                `Found results for "${debouncedSearchTerm}" in Class(es): ${classNumbersList}. Select a class from the dropdown to filter.`
              );
              const allDetails = sortedClasses.flatMap((c) =>
                Array.from(c.details)
              );
              const detailSummary =
                allDetails.slice(0, 3).join("; ") +
                (allDetails.length > 3 ? "..." : "");
              ...
              setClassDetailInput("");
              setMessage(
                `Found results for "${debouncedSearchTerm}" but no specific class details available.`
              );
            }
          } else if (hasSearchTerm && hasClassSelected) {
            setMessage(
              `Showing results for "${debouncedSearchTerm}" in Class ${selectedClassNumber}.`
            );
          } else if (!hasSearchTerm && hasClassSelected) {
    ...
    if (showClassDetailSearch && debouncedClassDetailSearchTerm.trim() !== "") {
      ...
      try {
          ...
          if (item.class_number && item.class_detail) {
            ...
            const matchingLines = cleanedDetail
              ...
              .filter((line) =>
                line
                  .toLowerCase()
                  .includes(debouncedClassDetailSearchTerm.toLowerCase())
              );
        ...
        if (classDetailsResultsArray.length > 0) {
          setMessage(
            `Found ${classDetailsResultsArray.length} class(es) containing "${debouncedClassDetailSearchTerm}"`
          );
        } else {
          setMessage(
            `No class details found containing "${debouncedClassDetailSearchTerm}".`
          );
        }
      } catch (err) {
          ...
          {!showClassDetailSearch ? (
            <>
              ...
              <div className="form-group">
                ...
                <div className="helper-text">
                  Search by Brand Name above to see relevant classes and details
                  here. Select a class from the dropdown to filter results.
                </div>
              </div>
                ...
                <div className="relevant-classes-info">
                  ...
                  <ul>
                    {relevantClasses.map((cls) => (
                      <li key={cls.number}>
                        <strong>Class {cls.number}:</strong>{" "}
                        {Array.from(cls.details).join("; ") ||
            ...
            <>
              ...
              <div className="form-group">
                ...
                <div className="helper-text">
                  Enter keywords to find which trademark classes include those
                  goods/services.
                </div>
              </div>
                  ...
                  <div className="class-detail-results">
                    ...
                    <ul>
                      {classDetailResults.map((cls, index) => (
                        <li key={index}>
                          <strong>Class {cls.number}:</strong>{" "}
                          {cls.details.slice(0, 2).join("; ")}
                          {cls.details.length > 2 ? "..." : ""}
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              {!loading &&
                ...
                debouncedClassDetailSearchTerm.trim() !== "" && (
                  <div className="info-message">
                    No matching classes found in sidebar for "
                    {debouncedClassDetailSearchTerm}".
                  </div>
                )}
            </>
          )}
            ...
            {(debouncedClassDetailSearchTerm || loading) && (
              <div className="search-header">
                <h2 className="search-title">
                  {loading
                    ? "Searching Class Details..."
                    : "Class Detail Results"}
                  {debouncedClassDetailSearchTerm && !loading
                    ? ` for "${debouncedClassDetailSearchTerm}"`
                    : ""}
                    ...
                    {classDetailResults.map((cls, index) => (
                      <li key={index} className="class-detail-item">
                        <div
                          ...
                          onClick={() =>
                            handleClassDetailResultClick(cls.number)
                          }
                            ...
                            <li key={i} className="matching-detail">
                              {debouncedClassDetailSearchTerm.trim() ? (
                                detail
                                  ...
                                  .map((part, j) =>
                                    part.toLowerCase() ===
                                    debouncedClassDetailSearchTerm.toLowerCase() ? (
            ...
            {!loading &&
              ...
              !message && (
                <div className="initial-message">
                  <p>
                    Enter class details (e.g., "software", "clothing") to find
          ...
          <>
              ...
              <div className="search-header">
                <h2 className="search-title">
                  ...
                  {debouncedSearchTerm && selectedClassNumber && !loading
                    ? " in "
                    : ""}
                  {selectedClassNumber && !loading
                    ? `Class ${selectedClassNumber}`
                    : ""}
                  {!loading &&
                    totalBrandResults > 0 &&
                    ` (${totalBrandResults} trademarks found)`}
                  {!loading &&
                    totalBrandResults === 0 &&
                    (debouncedSearchTerm || selectedClassNumber) &&
                    ` (0 trademarks found)`}
                </h2>
              </div>
            )}
              ...
              <>
                          ...
                          <div className="trademark-grid">
                            ...
                            {trademark.class_detail && (
                              <div className="full-width">
                                ...
                                {cleanDetailText(trademark.class_detail)}
                              </div>
                            )}
                          </div>
                        </div>
                ...
                {totalBrandResults > brandSearchLimit && (
                  <div className="pagination-controls">
                    <button
                      onClick={() =>
                        handleBrandPageChange(brandSearchCurrentPage - 1)
                      }
                    ...
                    <span>
                      Page {brandSearchCurrentPage} of {totalBrandPages}{" "}
                      (Results{" "}
                      {(brandSearchCurrentPage - 1) * brandSearchLimit + 1} -{" "}
                      {Math.min(
                        brandSearchCurrentPage * brandSearchLimit,
                        totalBrandResults
                      )}
                      )
                    </span>
                    <button
                      ...
                      disabled={
                        brandSearchCurrentPage === totalBrandPages || loading
                      }
                    >
                      Next
                    </button>
            ...
            {!loading &&
              ...
              !message && (
                <div className="initial-message">
                  <p>
                    Enter a brand name or select a class to search for
                    trademarks.
                  </p>
                  <NavLink
                    to={publicUser.tdclasses.TdClass01}
                    className="bg-primary text-white p-3 mt-4 rounded "
                    style={{
                      width: "100px",
                      height: "50px",
                      cursor: "pointer",
